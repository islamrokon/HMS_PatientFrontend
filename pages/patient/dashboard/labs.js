...
export default function GetUsers({ data }) {
      ...
      <ul>
        {data.map(item => (
          <li key={item.testid}>
            ...
            <Link href={"/patient/dashboard/laball/" + item.testid}><div className="font-bold text-blue-600/100 bg-gray-200 px-16 m-10"
            >{item.testname}</div></Link>
            </li>
        ))}
      </ul>
 ...
 export async function getServerSideProps() {
      ...
      const response = await axios.get('https://nestjsproject-production-364f.up.railway.app/Patient/labs');
